...
#
# Interact with bswck/skeleton (current version: https://github.com/bswck/skeleton/tree/0.0.2rc-140-gfb19b08).
#
# This file was generated from bswck/skeleton@0.0.2rc-140-gfb19b08.
# Instead of changing this particular file, you might want to alter the template:
# https://github.com/bswck/skeleton/tree/0.0.2rc-140-gfb19b08/project/scripts/skeleton.%7B%7Bsref%7D%7D.bash.jinja
#
# Usage:
#
...
make_context() {
	...
	COVERAGE_URL=$(cat <<- 'EOF'
		https://coverage-badge.samuelcolvin.workers.dev/redirect/bswck/backframe
	EOF
	)
	export DOCS_URL
	DOCS_URL=$(cat <<- 'EOF'
		https://backframe.readthedocs.io/en/latest/
	EOF
	)
	export PYPI_URL
	PYPI_URL=$(cat <<- 'EOF'
		https://pypi.org/project/backframe/
	EOF
	)
	export SKELETON
	SKELETON=$(cat <<- 'EOF'
	...
	SKELETON_REV=$(cat <<- 'EOF'
		https://github.com/bswck/skeleton/tree/0.0.2rc-140-gfb19b08
	EOF
	)
	export SREV
	SREV=$(cat <<- 'EOF'
	...
	SKELETON_AND_REF=$(cat <<- 'EOF'
		bswck/skeleton@0.0.2rc-140-gfb19b08
	EOF
	)
	export SNREF
	SNREF=$(cat <<- 'EOF'
	...
	GH_ENSURE_ENV=$(cat <<- 'EOF'
		jq -n '{"deployment_branch_policy": {"protected_branches": false,"custom_branch_policies": true}}' | gh api -H "Accept: application/vnd.github+json" -X PUT "/repos/bswck/backframe/environments/$1" --input -
	EOF
	)
    export LAST_REF="0.0.2rc-140-gfb19b08"
